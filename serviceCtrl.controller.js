...
function encryptData(dataInput, callback) {
	...
	try {
		...
		console.log('Input data to be encrypted=', dataInput);
		key.importKey(fs.readFileSync('Keys/public.pem'));
		const encrypted = key.encrypt(dataInput, 'base64');
		console.log('encrypted=', encrypted);
		encryptData_resp.status = 'SUCCESS';
		encryptData_resp.data = encrypted;
	...
	try {
		...
		dbInstance.onValue(dataObj, (snapShot)=>{
			...
			if(needRawData == 'true'){
				...
				key.importKey('Keys/private.pem', 'pkcs1');
				decrypted = key.decrypt(snapShot.val().data, 'utf8');
